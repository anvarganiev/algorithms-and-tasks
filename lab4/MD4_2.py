...
class MD4(object):
    ...
    def _add_chunk(self, chunk):
        self.count += 1
        X = list(struct.unpack("<16I", chunk) + (None,) * (80 - 16))  # < - little-endian, I - integer
        h = [x for x in self.h]
        # Round 1
        s = (3, 7, 11, 19)
        for r in range(16):
        ...
        for r in range(16):
            i = (16 - r) % 4
            h[i] = leftrotate(
                (h[i] + H(h[(i + 1) % 4], h[(i + 2) % 4], h[(i + 3) % 4]) + X[k[r]] + 0x6ed9eba1) % 2 ** 32, s[r % 4])

        for i, v in enumerate(h):
            self.h[i] = (v + self.h[i]) % 2 ** 32

    def add(self, data):
        message = self.remainder + data  #  остаток
        r = len(message) % 64
        if r != 0:
